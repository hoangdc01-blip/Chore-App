...
export default function RewardShop() {
  ...
  const handleRedeem = (rewardId: string) => {
    ...
    if (!reward) return
    const available = getAvailablePoints(effectiveMemberId)
    if (available < reward.cost) {
      showToast('Not enough points!', 'error')
      return
    }
    redeemReward(rewardId, effectiveMemberId)
    fireConfetti()
    const member = members.find((m) => m.id === effectiveMemberId)
    showToast(`${member?.name} redeemed ${reward.emoji} ${reward.name}! 🎟️`, 'success')
  }
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 pb-20 xl:p-6 xl:pb-20 space-y-4 xl:space-y-6">
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium transition-all min-h-[48px] border-2 ${
                    isSelected
                      ? `${color.bg} ${color.darkBg} ${color.text} ${color.darkText} border-transparent shadow-md scale-105`
      ...
      {/* Kid mode: show points */}
      {isKidMode && activeKidId && (
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <Star size={14} className="text-amber-600" fill="currentColor" />
          <span className="font-semibold text-amber-600">{getAvailablePoints(activeKidId)} points available</span>
        </div>
      )}
      ...
      {rewards.length === 0 ? (
        <div className="text-center py-12 text-muted-foreground">
          ...
          <p>{isKidMode ? 'No rewards available yet!' : 'No rewards yet. Add some rewards for your kids!'}</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
          {rewards.map((reward) => {
            const canAfford = effectiveMemberId ? getAvailablePoints(effectiveMemberId) >= reward.cost : false
            return (
              <div
                key={reward.id}
              ...
              >
                ...
                <button
                  ...
                  disabled={!effectiveMemberId || !canAfford}
                ...
                >
                  {!effectiveMemberId ? 'Select a kid' : canAfford ? 'Redeem!' : 'Not enough pts'}
                </button>
              </div>
            )
          })}
      ...
      {!isKidMode && <RewardDialog open={dialogOpen} onClose={() => setDialogOpen(false)} />}
    </div>
  )
}
